...
const Home: NextPage = () => {
  ...
  const getMoiActions = (): AccountAction[] => {
    const onSendInteraction = async (chainId: string, address: string) => {
      openRequestModal();
      await moiRpc.testSendInteraction(chainId, address);
    };
    const onSignInteractions = async (chainId: string, address: string) => {
      openRequestModal();
      await moiRpc.testSignInteractions(chainId, address);
    };
    const onSign = async (chainId: string, address: string) => {
      openRequestModal();
      await moiRpc.testSign(chainId, address);
    };
    const onAssetInvokeSign = async (chainId: string, address: string) => {
      openRequestModal();
      await moiRpc.testSignAssetInvoke(chainId, address);
    };
    const onAssetInvokeSend = async (chainId: string, address: string) => {
      openRequestModal();
      await moiRpc.testSendAssetInvoke(chainId, address);
    };

    const onParticipantCreate = async (chainId: string, address: string) => {
      openRequestModal();
      await moiRpc.testSendParticipantCreate(chainId, address);
    };

    const onSignParticipantCreate = async (
      chainId: string,
      address: string,
    ) => {
      openRequestModal();
      await moiRpc.testSignParticipantCreate(chainId, address);
    };

    const onSignAccountConfigure = async (chainId: string, address: string) => {
      openRequestModal();
      await moiRpc.testSignAccountConfigure(chainId, address);
    };

    const onSendAccountConfigure = async (chainId: string, address: string) => {
      openRequestModal();
    ...
    return [
      ...
      {
        method: "Sign Asset Invoke",
        callback: onAssetInvokeSign,
      },
      {
        method: "Send Asset Invoke",
        callback: onAssetInvokeSend,
      },
      {
        method: "Send Participant Create",
        callback: onParticipantCreate,
      },
      {
        method: "Sign Participant Create",
        callback: onSignParticipantCreate,
      },
      {
        method: "Sign Account Configure",
        callback: onSignAccountConfigure,
      },
      {
